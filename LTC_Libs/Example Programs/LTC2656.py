...
#
# Description:
#   This program demonstrates how to communicate with an LTC2656 16-bit Octal
# SPI DAC using a DC590B and demo board 1397A.
#
# Revision History:
...
def print_prompt():
    ...
    print "  5-Set the DAC to Zero"
    print "  6-Set the DAC to Full Scale"
    print "  7-Exit\n"
...
if dc590.count>0:    # Check for a connected DC590
    dc590.spi_mode() # Setup spi communications   
    while 1:
        print_prompt()
        data=get_integer("Enter Command: ",1,5)   
        if data==1:
            dc590.spi_write_word(command | address,code)  #update DAC via dc590
            #here is alternative way to send data
            #data_buffer=[0x3F,0x80,0x00]                 #set spi data
            #dc590.spi_write_buffer(data_buffer)          #write the data
            print "DAC updated"
        elif data==2:
            #DAC Code
            ...
            #DAC to zero
            dc590.spi_write_word(command | address,0x0000)
        elif data==6:
            #DAC to zero
            dc590.spi_write_word(command | address,0xFFFF)
        elif data==7:
            #Exit
            print "\nExiting Program"
            break              
